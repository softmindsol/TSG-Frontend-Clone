...
const OptionalMilestones = ({ singleDeal }) => {
  ...
  // ✅ Load data from backend when singleDeal changes
  useEffect(() => {
    if (
      singleDeal?.optionalMilestones &&
      singleDeal.optionalMilestones.length > 0
    ) {
      const c = singleDeal.optionalMilestones[0];
      reset({
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      await dispatch(updateOptionalMilestones({ dealId, data: fd }));
      await dispatch(getSingleDeal({ dealId })); // refresh redux store
      setIsEditing(false);
      toast.success("Optional milestone updated successfully!");
    } catch (error) {
      toast.error("Failed to update optional milestone!");
    }
  };

  // ✅ Cancel handler resets to backend values
  const handleCancel = () => {
    if (
      singleDeal?.optionalMilestones &&
      singleDeal.optionalMilestones.length > 0
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="grid grid-cols-1 my-4 sm:grid-cols-2 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            />
            {!isEditing && singleDeal?.optionalMilestones?.[0]?.file?.url && (
              <p className="text-sm text-gray-600 mt-1">
                Current file:{" "}
                <a
                  href={singleDeal.optionalMilestones[0].file.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-blue-600 underline"
                >
                  {singleDeal.optionalMilestones[0].file.filename ||
                    "View File"}
                </a>
              </p>
            )}
...
export default OptionalMilestones;
